...
public class MyButton5 {
    ...
    public int whichPic;
    public MyButton5(int x, int y, int z) {
        this.x = x;
        this.y = y;
        this.whichPic = z;
        ...
        for (int i=0; i<2; i++ ) {
            ...
            // 평가결과보기에서 왼쪽, 오른쪽 버튼 이미지, 닫기 버튼, 전체 삭제 버튼
            if (whichPic == 12 || whichPic == 27 || whichPic == 28 || whichPic == 30) {
                int xWidth = StudyView5.Width / 11;
                int yWidth = xWidth;
    ...
    public MyButton5(int x, int y, int z, int w) {
        this.x = x;
        this.y = y;
        this.whichPic = z;

        // 문제수 버튼 10,20,25,50,100 문제
        buttonImage[0] = BitmapFactory.decodeResource(StudyView5.mContext.getResources(), R.drawable.test00+whichPic);
